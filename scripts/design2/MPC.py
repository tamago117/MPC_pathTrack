...
class MPC:
    def __init__(self, T, N, max_v, max_w, lane_width):
        ...
        self.Q  = [0.0, 1.0]       # state weights[x, y, angle, v]
        self.Qf = [1.0, 1.0]       # terminal state weights
        self.R  = [0.01, 0.01]          # input weights[v, w]
        self.V_W = 1.0

        self.max_v_input = max_v
        self.max_w_input = max_w
        ...
        for k in range(self.N):
            ...
            w += [Uk]
            lbw += [-self.max_v_input, -self.max_w_input]
            ubw += [self.max_v_input, self.max_w_input]
            w0 += [0, 0]
            lam_x0 += [0, 0]
